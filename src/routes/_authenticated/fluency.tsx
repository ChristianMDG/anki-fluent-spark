...
function FluencyPage() {
  // `/fluency/journal` is registered as a CHILD route of `/fluency` (see
  // routeTree.gen.ts: AuthenticatedFluencyRouteWithChildren). A child route
  // only renders if the parent's component includes an <Outlet />. This
  // component previously rendered its own full practice UI unconditionally
  // and never mounted an <Outlet />, so navigating to "Journal" changed the
  // URL but showed nothing new — the journal page was simply never given a
  // place to render. Delegate entirely to the child when its path is active.
  // NOTE: the check itself must come AFTER every other hook below (all hooks
  // must run unconditionally on every render, per the Rules of Hooks) — only
  // the JSX branch it guards is conditional.
  ...
  if (pathname.startsWith("/fluency/journal")) {
    return <Outlet />;
  }
...
function useFluencyStreak() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Local-day bucketing (see note in fluency.journal.tsx groupByDay): a
      // session completed late at night local time must not be pushed into
      // "yesterday" just because its UTC date differs.
      const keyOf = (d: Date) => `${d.getFullYear()}-${d.getMonth()}-${d.getDate()}`;
      const days = new Set(data.map((r) => keyOf(new Date(r.completed_at as string))));
      let streak = 0;
