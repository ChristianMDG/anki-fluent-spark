...
import { Book, Plus, Check, Trash2, Volume2, ChevronDown, AlertTriangle, X, Fingerprint, Bookmark, BookmarkCheck } from "lucide-react";
...
export function VocabCard({
  ...
  async function handleConfirmDelete() {
    ...
    const ok = await confirmDialog({
      ...
      description: `Le mot "${card.word}" sera définitivement retiré de votre lexique de révision.`,
      ...
      >
        ...
        <div className="flex items-center justify-between h-5 mb-4 shrink-0 relative z-[1]">
          <div className="flex items-center gap-1.5 text-muted-foreground/60">
            ...
            <span className="label-mono text-[9px] tracking-[0.2em] whitespace-nowrap">FILE #{dossierNumber}</span>
          </div>
          ...
          <div className="h-5 flex items-center">
            {needsReview ? (
              <span className="label-mono text-[9px] tracking-[0.15em] text-amber-300 border border-amber-500/30 rounded px-1.5 py-0.5 bg-amber-500/10 flex items-center gap-1 whitespace-nowrap">
                <AlertTriangle size={10} className="shrink-0" /> À REVOIR
              </span>
            ) : !exported ? (
        ...
        <header className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3 shrink-0 relative z-[1] min-h-[72px] max-h-[72px] overflow-hidden">
          <div className="min-w-0 flex flex-col justify-between h-full">
            <div className="flex items-center gap-2 min-h-[28px] max-h-[28px] relative">
              <h2 className="text-xl md:text-2xl font-bold truncate text-white select-all pr-7" title={card.word}>
                {card.word}
              </h2>
              <button
            ...
            <div className="space-y-3.5 overflow-y-auto pr-1 max-h-[220px] custom-scrollbar">
              {card.definition && (
                <section className="pl-3 border-l-2 border-[color:var(--color-crimson)]/40 break-words">
                  <p className="label-mono mb-0.5 text-[9px] tracking-[0.15em] text-muted-foreground/60 select-none">DEFINITION</p>
              ...
              {card.french && (
                <section className="pl-3 border-l-2 border-[color:var(--color-gold)]/40 break-words">
                  <p className="label-mono mb-0.5 text-[9px] tracking-[0.15em] text-muted-foreground/60 select-none">FRANÇAIS</p>
                  <p className="text-[color:var(--color-gold)] text-sm" dangerouslySetInnerHTML={{ __html: card.french }} />
                </section>
              )}

              {!compact && card.grammar && (
                <section className="pl-3 border-l-2 border-white/10 break-words">
                  <p className="label-mono mb-0.5 text-[9px] tracking-[0.15em] text-muted-foreground/60 select-none">GRAMMAR</p>
                  <div className="text-sm text-neutral-300" dangerouslySetInnerHTML={{ __html: grammarToHtml(card.grammar) }} />
                </section>
              )}

              {card.examples && (
                <section className="pl-3 border-l-2 border-white/10 break-words">
                  <p className="label-mono mb-0.5 text-[9px] tracking-[0.15em] text-muted-foreground/60 select-none">EXAMPLES</p>
                  <div className="text-sm text-neutral-300 leading-relaxed" dangerouslySetInnerHTML={{ __html: examplesToHtml(card.examples) }} />
                </section>
              )}

              {!compact && card.cloze && (
                <section className="pl-3 border-l-2 border-[color:var(--color-crimson)]/20 break-words">
                  <p className="label-mono mb-0.5 text-[9px] tracking-[0.15em] text-muted-foreground/60 select-none">CLOZE</p>
                  <div className="text-sm text-neutral-300" dangerouslySetInnerHTML={{ __html: clozeToHtml(card.cloze) }} />
                </section>
              )}
            </div>
          ) : (
          ...
          <div className="flex items-center justify-between h-8 min-h-[32px] max-h-[32px]">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <button
                ...
                title={!exported ? "Retirer de la file d'export" : "Ajouter à l'export"}
            ...
            <div className="pt-3 mt-3 border-t border-white/5 space-y-2">
              <div className="flex flex-wrap gap-1 max-h-[52px] overflow-y-auto pr-0.5 custom-scrollbar">
                {tags.map((t) => (
                  ...
                  >
                    {t}
                    <button onClick={() => toggleTag(t)} className="hover:text-red-400 transition-colors shrink-0">
                      <X size={10} />
                    </button>
                  </span>
                ))}
